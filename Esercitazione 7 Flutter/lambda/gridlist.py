...
def lambda_handler(event, context):
    try:
        ...
        xpath_query = root.findall(xpath_exp)

        grid = {}

        for x in xpath_query:
            family = x.find('.//Person/Name/Family')
            given = x.find('.//Person/Name/Given')
            start = x.find('.//Start/StartTime')
            org = x.find('.//Organisation/Name')
            country = x.find('.//Organisation/Country')
            bib = x.find('.//Start/BibNumber')
